...
function Header()
{
    ...
    $this->SetY(15);
    $this->Cell(300,10,'Unserviceable Assets Report');
    $this->SetXY(10,25);
...
$pdf->SetTitle('List of Unserviceable Assets ');
//$pdf->Header();
...
$olddep='';
    ...
    $asset_qry = $conn->query("SELECT *,d.name as departmentname FROM assets a LEFT JOIN employee e on a.assetassignee=e.id LEFT JOIN department d on a.department_id = d.id LEFT JOIN category c on a.category_id = c.id WHERE status='UNSERVICEABLE' ORDER BY d.name ") or die($conn->error);
