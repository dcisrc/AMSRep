...
function Header()
{
    ...
    $this->SetY(15);
    $this->Cell(300,10,'Asset Management System - Assigned Assets Per Employee Report');
    $this->SetX(140);
...
$oldemp='xxx';
    ...
    $asset_qry = $conn->query("SELECT *,a.assetassignee as employee_code, concat(e.lastname,', ',e.firstname) as empname  FROM assets a LEFT JOIN employee e on a.assetassignee=e.id LEFT JOIN department d on a.department_id = d.id LEFT JOIN category c on a.category_id = c.id ORDER BY a.assetassignee") or die($conn->error);
